describe('Testing Angular Acme Bank', () => {
  ...
  cy.getByClass('mat-horizontal-content-container')
    .find('form')
    .then(form => {
      cy.wrap(form)
        .find('[placeholder="First name, Last name"]')
        .type('Daniela de Aguiar Jr.');
      cy.wrap(form)
        .find('[placeholder="Street, City, Zip Code"]')
        .type('Tv Djalma Dutra');
      cy.wrap(form)
        .find('[class="mat-select-arrow-wrapper"]')
        .click();
  ...
  cy.get('.mat-horizontal-content-container')
    .find('.mat-horizontal-stepper-content')
    .then(form => {
      const inputs = form.find('[class="mat-form-field-wrapper"]');
      const labels = form.find('mat-label');

      cy.wrap(inputs[4])
        .find('[class="mat-form-field-infix"]')
        .type('50000');
